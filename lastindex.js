...
async function fetchPostData(baseUrl, url, requestId) {
    ...
    try {
        ...
        // Attempt to find the API URL within the HTML (customize based on actual structure)
        const apiUrlMatch = html.match(/window\.__INITIAL_STATE__\s*=\s*({.*});/);
        let dynamicContent = '';

        if (apiUrlMatch) {
            const initialState = JSON.parse(apiUrlMatch[1]);

            // Assuming the API URL is within the initialState (customize based on actual structure)
            const apiUrl = initialState.someProperty.apiUrl; // Adjust this path as needed
            
            if (apiUrl) {
                console.log(`[${requestId}] Fetching dynamic content from API: ${apiUrl}`);
                try {
                    const apiResponse = await axios.get(apiUrl);
                    const apiData = apiResponse.data;

                    // Assuming the post content is within content.render (customize based on actual structure)
                    if (apiData && apiData.content && apiData.content.render) {
                        dynamicContent = apiData.content.render;
                    }

                } catch (apiError) {
                    console.error(`[${requestId}] Error fetching dynamic content from API: ${apiError.message}`);
                }
            }
        } else {
            console.warn(`[${requestId}] API URL not found in the initial HTML for dynamic content.`);
        }

        // Combine static and dynamic content
        const completePostContent = postContent + dynamicContent;

        // Extract links from completePostContent
        const links = [];
        const combinedContent$ = cheerio.load(completePostContent);

        combinedContent$('a').each((index, element) => {
            const href = combinedContent$(element).attr('href');
            if (href && href !== '#' && !href.startsWith('#') && !href.includes('postshare')) { // Exclude links containing 'postshare'
                const linkText = combinedContent$(element).text().trim();
        ...
        const data = {
            publishDate,
            authorName,
            postContent: completePostContent,
            externalLinks,
            internalLinks,
            requestUrl: url,
            error: null,
